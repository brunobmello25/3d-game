...
func main() {
	...
	for !rl.WindowShouldClose() { // Detect window close button or ESC key
		...
		if rl.IsKeyPressed(rl.KeyOne) {
			...
			fmt.Println("Camera mode set to FREE")
		}
		if rl.IsKeyPressed(rl.KeyTwo) {
			cameraMode = rl.CameraFirstPerson
			camera.Up = rl.NewVector3(0.0, 1.0, 0.0) // Reset roll
			fmt.Println("Camera mode set to FIRST PERSON")
		}
		if rl.IsKeyPressed(rl.KeyThree) {
			cameraMode = rl.CameraThirdPerson
			camera.Up = rl.NewVector3(0.0, 1.0, 0.0) // Reset roll
			fmt.Println("Camera mode set to THIRD PERSON")
		}
		if rl.IsKeyPressed(rl.KeyFour) {
			cameraMode = rl.CameraOrbital
			camera.Up = rl.NewVector3(0.0, 1.0, 0.0) // Reset roll
			fmt.Println("Camera mode set to ORBITAL")
		}
